...
export function DomainCheckerSection({
  ...
  return (
    <section
      ...
      className={`relative px-4 sm:px-8 md:px-12 lg:px-24 overflow-hidden z-10 ${isLivePreview
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden z-0" aria-hidden="true">
        <div
          className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[70vw] h-[35vw] rounded-full blur-[160px] pointer-events-none transition-opacity ${isLightMode ? 'bg-neutral-200/40 opacity-30' : 'bg-white/[0.03] opacity-25'
            }`}
        />
      </div>

      <div
        className={`relative z-10 max-w-5xl mx-auto flex flex-col items-center text-center w-full ${isLivePreview ? 'my-auto' : ''
          }`}
      >
        ...
        >
          <span className="block">Inspect Any Domain</span>
          <span className="block">in Real-Time.</span>
        </h2>
        <p
          className={`text-sm sm:text-base md:text-lg font-normal leading-relaxed max-w-2xl mx-auto mb-8 sm:mb-10 text-center ${isLightMode ? 'text-neutral-600' : 'text-neutral-400'
            }`}
        >
        ...
        <p
          className={`text-center font-mono text-[11px] sm:text-xs tracking-wider uppercase mt-8 select-none transition-colors ${isLightMode ? 'text-neutral-500' : 'text-neutral-400'
            }`}
        >
